...
do 
    query_results+=("$line")
done < <(mysql --user="$mysql_user_id" --password="$mysql_passwd" "$mysql_db_name" --execute="SELECT c.class_name, p.patient_name, s.sample_type, s.is_pair, s.replicates, f.file_path, f.file_name, f.pair_number, f.replicate_number, e.* FROM experiments e INNER JOIN experiment_files ef ON e.id = ef.experiment_id INNER JOIN files f ON f.id = ef.file_id INNER JOIN sample_groups s ON s.id = f.sample_group_id INNER JOIN patients p ON p.id = s.patient_id INNER JOIN classes c ON c.id = p.class_id WHERE e.uid='$uid';")

# above will extract following tables
#######################################################################################################################################################
#+------------+--------------+-------------+---------+------------+-----------------------------------------------------------------+-----------+-------------+------------------+----+------------------------------------------------+---------------+
