...
declare -A exp_info
																					...
																					#temp_fields=("f1" "f2" "f3")
																					#datas=("11" "22" "33")
																					#
																					#for (( i=0; i<2; i++ )); do 
																					#	for key in "${temp_fields[@]}"; do
																					#		echo $i, $key, ${datas[$i]}
																					#		newmap[$i,$key]=${datas[$i]}
																					#		declare -p newmap
																					#	done
																					#done
																					#
...
# variables
class_name_list=(); patient_name_list=(); sample_type_list=(); sample_format_list=(); is_pair_list=(); replicate_list=(); file_path_list=(); file_name_list=(); pair_num_list=(); replicate_num_list=(); id_list=(); uid_list=(); analysis_type_list=() 
field_list=()
...
while read line; do 
	...
	((i++))
	for (( field_index=0; field_index<${#field_list[@]}; field_index++ )); do
		echo "here"
		echo $field_index
		exp_info[$i,${field_list[$field_index]}]=${fields[$field_index]}
	done
	
	# add each fields to lists
	class_name_list+=(${fields[0]}); patient_name_list+=(${fields[1]}); sample_type_list+=(${fields[2]}); sample_format_list+=(${fields[3]}); is_pair_list+=(${fields[4]}); replicate_list+=(${fields[5]}); file_path_list+=(${fields[6]}); file_name_list+=(${fields[7]}); pair_num_list+=(${fields[8]}); replicate_num_list+=(${fields[9]}); id_list+=(${fields[10]}); uid_list+=(${fields[11]}); analysis_type_list+=(${fields[12]})
...
<<'COMMENT' # would be deleted
						...
						for (( i=0 ; i<${#query_results[@]} ; i++ )); do
								...
								#IFS=$'\t' read -ra fields <<< "${query_results[$i]}"	
