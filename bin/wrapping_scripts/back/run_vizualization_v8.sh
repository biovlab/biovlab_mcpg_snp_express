...
ME_input_type=$me_sample_format			# 0: MBD-Seq / MeDIP-Seq, 1: "Infinium 27k", 2:"Infinium 450k", 3:"BS-seq"
GE_input_type=$ge_is_array 							# 1:array, 2:RNAseq
MU_input_type=$mu_sample_format					# 0:DNAseq, 1:RNAseq, 2:HumanHap


# NOTE : THIS SHOULD BE SAME FOR ALL ME, GE, MU
type_kind=(${ge_type_kind[@]})
type_list=(${ge_type_list[@]})
sample_num_in_class=($(echo ${ge_type_list[@]} | tr ' ' '\n' | sort | uniq -c | awk '{print $1}'| tr '\n' ' '))
...
elif [ $ME_input_type -eq 3 ] ; then # BS
