...
web_accessible_url=$rnaseq_result_dir"/web/"
mkdir -p $web_accessible_url
...
#	java -Xmx20g -jar $picard_dir/BuildBamIndex.jar I=$second_star_out_bam_sorted
  ...
  java -Xmx20g -jar $picard_dir/MarkDuplicates.jar I=$second_star_out_bam_sorted O=$second_star_out_dedupped_bam  CREATE_INDEX=true VALIDATION_STRINGENCY=SILENT M=output.metrics 
	
  echo "[INFO] Split'N'Trim and reassign mapping qualities" # Split'N'Trim and reassign mapping qualities
  java -Xmx20g -jar $gatk_dir/GenomeAnalysisTK.jar -T SplitNCigarReads -R $REF_HUMAN -I $second_star_out_dedupped_bam -o $split_bam -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 -U ALLOW_N_CIGAR_READS
