...
# TODO: Change as ME vs GE to speed up
	...
	for (( i=0; i<${#vcf_list[@]}; i++ )); do
		temp_filename_only=`basename \${vcf_list[$i]}`
		snp_level_file=$mutation_result_dir/$temp_filename_only".bed"
	...
	for (( k=0; k<${#type_kind[@]}; k++ )); do 
		...
		for (( i=0; i<${#vcf_list[@]}; i++ )); do
			if [ ${type_list[$i]} == ${type_kind[$k]} ]; then
				SNP_MGD_LIST+=($integ_result_dir"/"`basename \${vcf_list[$i]}`".SNP_RNGs.MGDrefS")
				SNP_MGD_LIST_all+=($integ_result_dir"/"`basename \${vcf_list[$i]}`".SNP_RNGs.MGDrefS")
...
# 4.2.4 DEG vs DSNP, pairwise comparison
	...
	for (( i=0; i<${#type_kind[@]}; i++ )); do 
		for (( j=$i+1; j<${#type_kind[@]}; j++ )); do 
			...
			for (( k=0; k<${#vcf_list[@]}; k++ )); do 

				snp_file=$integ_result_dir"/"`basename \${vcf_list[$k]}`".SNP_RNGs.MGDrefS"
