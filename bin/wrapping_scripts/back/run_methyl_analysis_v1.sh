...
if [ $bs_mbd == "bs" ]; then
	...
	for (( k=0; k<${#BS_TYPE_KIND[@]}; k++)); do
		...
		for (( l=$k+1; l<${#BS_TYPE_KIND[@]}; l++ )); do
			...
			Rscript $bin_dir/comp_stat.r -s `join "," ${first_sample_class_list[@]}`","`join "," ${second_sample_class_list[@]}` -i `join "," ${first_sample_class_name[@]}`","`join "," ${second_sample_class_name[@]}` -c $class_ids -r $methylkit_from_bs_result_dir -w $window_size -n FALSE --out_prefix $work_file &
			let count+=1
			[[ $((count%$NUM_CPUS)) -eq 0 ]] && wait
		done
	done; wait
...
# for testing
	...
	for (( k=0; k<${#MBD_TYPE_KIND[@]}; k++ )); do 
		...
		for (( l=$k+1; l<${#MBD_TYPE_KIND[@]}; l++ )); do
			...
			Rscript $bin_dir/medips_pair_parallel.R -P $NUM_CPUS -a `join ";" ${first_sample_class_name[@]}` -b `join ";" ${second_sample_class_name[@]}` -r $medips_from_mbd_result_dir -p ${MBD_TYPE_KIND[$k]}"_vs_"${MBD_TYPE_KIND[$l]} -w $window_size --pvalue $pval_threshold
