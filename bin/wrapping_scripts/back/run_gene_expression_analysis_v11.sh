...
final_result_dir=$final_result_root_dir/gene-expression
	...
	# FOR WITH REPLICATES
		for (( k=0; k<${#type_kind[@]}; k++ )); do 
			...
			for (( l=$k+1; l<${#type_kind[@]}; l++ )); do
				...
				$R_DIR/Rscript $bin_dir"/"limma.R -a `my_join ";" ${first_sample_class_name[@]}` -b `my_join ";" ${second_sample_class_name[@]}` -r $cel_result_dir -l `my_join ";" ${sample_class_list[@]}` -p $work_file --pvalue $P_VALUE_CUT
				# get deg
<<'COMMENT'
...
COMMENT
				...
				sh $bin_dir/gsea3.sh $cel_result_dir/$work_file".DEG.list" $cel_result_dir "GENE_SYMBOL" $work_file".DEG.list"
				...
				'NR==1{next;}{OFS="\t"; if ($3 > 5) print $2, red4;
					...
					else print $1, white;}'  $cel_result_dir/$work_file".deg" > $cel_result_dir/$work_file".deg.KEGG"
	...
	# only extract expression level since no replicates
		for (( i=0; i<${#cel_list[@]}; i=i+2 )); do
			...
			$R_DIR/Rscript $bin_dir"/"limma.R -a ${cel_list[$i]} -b ${cel_list[$i+1]} -r $cel_result_dir -l $sample_num_list --pvalue $P_VALUE_CUT
		done
...
#		output_prefix=${sample_prefix_list[$i]}"_"
	...
	# copy all final results
	cp $deg_analysis_dir/*.DEG.* $deg_analysis_dir/*.KEGG $deg_analysis_dir/*.UP_DOWN $final_result_dir
fi
