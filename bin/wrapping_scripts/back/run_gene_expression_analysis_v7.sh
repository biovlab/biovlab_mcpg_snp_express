...
for (( i=0; i<$total_num_tuple; i++ )); do
	echo ${exp_info[$i,$SAMPLE_TYPE]} # SAMPLE_TYPE is defined in env.sh
done



# Go or Quit
if [ ${exp_info[0,$EXPRESSION_TYPE]} == "0" ]; then
	exit
fi

<<'COMMENT' # no more needed? then delete this kk
...
for (( i=0; i<$total_num_tuple; i++ )); do
	...
	# replicate	
	if [ $replicate_exist -eq -1 ] && [ ${exp_info[$i,$SAMPLE_TYPE]} == "Expression" ] && [ ${exp_info[$i, $REPLICATES]} != "1" ]; then
		replicate_exist=1
	fi

	# array or RNA-seq
	if [ $is_array -eq -1 ] && [ ${exp_info[$i,$SAMPLE_TYPE]} == "Expression" ] && [ ${exp_info[i,$SAMPLE_FORMAT]} == "Microarray" ]; then
		is_array=1
	fi

	# single-end or paired-end
	if [ $single_pair == ""] && [ ${exp_info[$i,$SAMPLE_TYPE]} == "Expression" ] && [ ${exp_info[0,$EXPRESSION_TYPE]} == "1" ]; then
		single_pair="single"
	fi

	# all set, break
	if [ $replicate_exist -ne -1 ] && [ $is_array -ne -1 ] && [ $single_pair != "" ]; then
		break
	fi
done
...
# input list
cel_list=() # for Microarray
sample_fq1_list=() # for RNA-seq pair-1
sample_fq2_list=() # for RNA-seq pair-2

if [ $is_array -eq 1 ]; then

	for (( i=0; i<$total_num_tuple; i++ )); do
	  if [ ${exp_info[$i,$SAMPLE_TYPE]} == "Expression" ]; then
	...
	# FOR WITH REPLICATES
		for (( k=0; k<${#type_kind[@]}; k++ )); do 
			...
			for (( l=$k+1; l<${#type_kind[@]}; l++ )); do
				...
				tail -n+2 $cel_result_dir/${type_kind[$k]}"_vs_"${type_kind[$l]}".deg" | cut -f2 | sort > $cel_result_dir/${type_kind[$k]}"_vs_"${type_kind[$l]}".DEG.list"
			done
		done
	else
	...
	# only extract expression level since no replicates
		...
		for (( i=0; i<${#cel_list[@]}; i++ )); do 
			for (( j=i+1; j<${#cel_list[@]}; j++ )); do 
				...
				cut -f1  $cel_result_dir"/"${type_list[$i]}"_vs_"${type_list[$j]}".deg" | sort > $cel_result_dir"/"${type_list[$i]}"_vs_"${type_list[$j]}".DEG.list"
