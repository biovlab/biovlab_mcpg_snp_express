...
source `dirname $0`/parse_exp_info_v4.sh
...
cel_list=(${me_array_list[@]})
type_kind=(${me_type_kind[@]})
type_list=(${me_type_list[@]})
sample_fq1_list=(${me_sample_fq1_list[@]})
sample_fq2_list=(${me_sample_fq2_list[@]})
...
if [ $sample_format -eq 3 ]; then # BS
	...
	for (( i=0; i<${#bs_r1_list[@]}; i++ )); do
		sh $bin_dir/bs.sh ${bs_r1_list[$i]} ${bs_r2_list[$i]} $pair_single_flag $bs_result_dir $adapter_seq &
		let count+=1
	...
	for (( i=0; i<${#bs_r1_list[@]}; i++ )); do
		...
		# run METHYLKIT
		Rscript $bin_dir/single_stat.r -s $temp_input -i $input_fq1_filename_only -c $i -r $methylkit_from_bs_result_dir -w $window_size &
	...
	for (( k=0; k<${#BS_TYPE_KIND[@]}; k++)); do
		...
		for (( l=$k+1; l<${#BS_TYPE_KIND[@]}; l++ )); do
			...
			Rscript $bin_dir/comp_stat.r -s `my_join "," ${first_sample_class_list[@]}`","`my_join "," ${second_sample_class_list[@]}` -i `my_join "," ${first_sample_class_name[@]}`","`my_join "," ${second_sample_class_name[@]}` -c $class_ids -r $methylkit_from_bs_result_dir -w $window_size -n FALSE --out_prefix $work_file &
			let count+=1
			[[ $((count%$NUM_CPUS)) -eq 0 ]] && wait
		done
	done; wait
	...
	ls $methylkit_from_bs_result_dir/*.sam.bed | xargs -I {} sh -c 'tail -n +2 $1 | sort -k1,1 -k2,2n - > $1.sorted' -- {}
	...
	for (( i=0; i<${#bs_r1_list[@]}; i++ )); do
		...
		paste <(tail -n+2 $MR_coverage_input) <(tail -n+2 $MR_input) | cut -f1,2,3,4,8 | sort -k1,1 -k2,2n | awk 'BEGIN{ print "chr\tbin_start\tbin_end\tmethyl_count\tmethyl_level"}{print $0}' > $methylkit_from_bs_result_dir/$input_fq1_filename_only".met_level"
	...
	for (( i=0; i<${#mbd_r1_list[@]}; i++ )); do
		sh $bin_dir/mbd.sh ${mbd_r1_list[$i]} ${mbd_r2_list[$i]} $pair_single_flag $mbd_result_dir		# align by STAR, so no need to be parallelization.
	done
...
# for testing
	...
	for (( k=0; k<${#MBD_TYPE_KIND[@]}; k++ )); do 
		...
		for (( l=$k+1; l<${#MBD_TYPE_KIND[@]}; l++ )); do
			...
			Rscript $bin_dir/medips_pair_parallel.R -P $NUM_CPUS -a `my_join ";" ${first_sample_class_name[@]}` -b `my_join ";" ${second_sample_class_name[@]}` -r $medips_from_mbd_result_dir -p ${MBD_TYPE_KIND[$k]}"_vs_"${MBD_TYPE_KIND[$l]} -w $window_size --pvalue $pval_threshold
...
<<'COMMENT'
	...
	for (( i=0; i<${#mbd_list[@]}; i++ )); do
		for (( j=$i+1; j<${#mbd_list[@]}; j++ )); do
			...
			Rscript $bin_dir/medips_pair.R -a $mbd_result_dir"/"$temp_filename_only1".sorted.bam" -b $mbd_result_dir"/"$temp_filename_only2".sorted.bam" -r $medips_from_mbd_result_dir -p $temp_filename_only1"_vs_"$temp_filename_only2 &
...
COMMENT
	...
	# for testing
		#Rscript /data/project/mcpg/bin/medips_pair.R -a $sample1_list -b $sample2_list -r $medips_from_mbd_result_dir -p "test"
