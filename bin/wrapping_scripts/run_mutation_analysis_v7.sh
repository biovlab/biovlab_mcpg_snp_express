...
#---------------------------------------------------------------------------------------------------
	...
	bash $bin_dir"/get_snp_count_per_window.sh" $filtered_vcf_out $runDir_2pass $lib_dir/genome_wide_bed_window
  ...
  # create redirect html
  echo -ne '<html><head><meta http-equiv="refresh" content="0; url='> $redirect_SNP_html
  echo -ne "http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg19&position=chr1:1-10000000&hgct_customText=track%20type=vcfTabix%20name="$output_prefix"SNP%20description="$output_prefix"SNP%20visibility=dense%20bigDataUrl="$WEB_ACCESSIBLE_LOC""$filtered_vcf_out_filename_only".gz" >> $redirect_SNP_html
  echo -e '" /></head><body></body></html>' >> $redirect_SNP_html
