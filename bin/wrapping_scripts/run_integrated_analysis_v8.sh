...
final_result_dir=$final_result_root_dir/inte
...
#	done
	...
	echo -e "#chr\tbin_start\tbin_end\t$header_string\trange_chr\trange_start\trange_end\tstrand\tgene_symbol\trange_kind\trefseq" > $ME_all_merged".txt"
...
# TODO: Change as ME vs GE to speed up
	...
	header_string=$(IFS=$'\t'; echo "${header_list[*]}")
	header_string2=$(IFS=$'\t'; echo "${header_list2[*]}")

	echo -e "#chr\tbin_start\tbin_end\t$header_string\t$header_string2\trange_chr\trange_start\trange_end\tstrand\tgene_symbol\trange_kind\trefseq" > $integ_result_dir/SNP_MGD.txt
...
# methylation file for tfbs-specific methyl level
 ...
 # average promoter methylation
	...
	# GE genesymbol
		...
		awk 'BEGIN{FS=OFS="\t"}{printf "%s", $1; for(i=2;i<=NF;i=i+2){printf "\t%f", $(i)}; printf "\n"}' $ge_genesymbol_avg_exp_temp > $ge_genesymbol_MGD

# revise for average promoter methyl
	tfbs_avg_methyl_GE_COR=$integ_result_dir/"TFBS_methyl_AVG_TF_GE_MGD_COR"
	awk 'BEGIN{FS=OFS="\t"}FILENAME==ARGV[1]{exp_arr[$1]=$0} FILENAME==ARGV[2]{if($1 in exp_arr){print $0,exp_arr[$1]}}' $ge_genesymbol_MGD $tfbs_avg_methyl_TF_INFO_filtered | python $bin_dir/pearson.py - 2 $((3+${#type_list[@]})) ${#type_list[@]} 0 > $tfbs_avg_methyl_GE_COR
...
# methyl average for cytoscape
	awk -v sample_num_str=`my_join "," ${TYPE_LEN[@]}` 'BEGIN{FS=OFS="\t"; num=split(sample_num_str, arr, ",")}{printf "%s\t%s\t", $1,$2; sum=0;start_index=4; for(i=1;i<=num;i++){sum=0;for(j=1;j<=arr[i];j++){sum=sum+$(start_index+j)};printf "%f\t", sum/arr[i] ;start_index=start_index+arr[i]};printf "\n"}' $integ_result_dir/TFBS_DEG_DMR > $integ_result_dir/TFBS_DEG_DMR_me_avg
  ...
  awk 'BEGIN{FS=OFS="\t"} FILENAME==ARGV[1]{arr[$1]=$0;next}
	...
	cp $ALL_avg_for_cyto_data $final_result_dir
