...
final_result_dir=$final_result_root_dir/mu
...
if [ "$data_type" -eq "0" ] || [ "$data_type" -eq "1" ] ; then	
	...
	#	java -Xmx20g -jar $picard_dir/BuildBamIndex.jar I=$second_star_out_bam_sorted
 	 ...
 	 java -jar -Xmx"$mem_for_gatk"g $picard_dir/MarkDuplicates.jar I=$second_star_out_bam O=$second_star_out_dedupped_bam  CREATE_INDEX=true VALIDATION_STRINGENCY=SILENT M=output.metrics
	  ...
	  java -jar -Xmx"$mem_for_gatk"g $gatk_dir/GenomeAnalysisTK.jar -T HaplotypeCaller -R $REF_HUMAN -I $split_bam -dontUseSoftClippedBases -stand_call_conf 20.0 -stand_emit_conf 20.0 -o $vcf_out
...
#---------------------------------------------------------------------------------------------------
	...
	#	for (( i=0; i<${#vcf_list[@]}; i++)); do
		...
		$bcftools_dir/bcftools merge $first_sample_vcf_string -O z --force-samples > $mut_mapping_dir"/"${type_kind[$k]}".vcf.MGD.gz"
	
		# indexing
		$tabix_dir/tabix -p vcf $mut_mapping_dir"/"${type_kind[$k]}".vcf.MGD.gz"
	done
	...
	for (( k=0; k<${#type_kind[@]}; k++ )); do
		for (( l=$k+1; l<${#type_kind[@]}; l++ )); do
			...
			first_vcf_file=$mut_mapping_dir"/"${type_kind[$k]}".vcf.MGD.gz"
			second_vcf_file=$mut_mapping_dir"/"${type_kind[$l]}".vcf.MGD.gz"
