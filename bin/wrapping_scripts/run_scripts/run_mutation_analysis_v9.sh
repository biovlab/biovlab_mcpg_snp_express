...
if [ "$data_type" -eq "0" ] || [ "$data_type" -eq "1" ] ; then	
	...
	for (( i=0; i<${#sample_fq1_list[@]}; i++ )); do
	  ...
	  java -jar -Xmx"$SYS_MAX_MEM"g $picard_dir/AddOrReplaceReadGroups.jar I=$second_star_out_sam O=$second_star_out_bam SO=coordinate RGID=id RGLB=library RGPL=platform RGPU=machine RGSM=sample

	  echo "[INFO] Mark duplicates"
 	 	java -jar -Xmx"$SYS_MAX_MEM"g $picard_dir/MarkDuplicates.jar I=$second_star_out_bam O=$second_star_out_dedupped_bam  CREATE_INDEX=true VALIDATION_STRINGENCY=SILENT M=output.metrics

	  echo "[INFO] Split'N'Trim and reassign mapping qualities" # Split'N'Trim and reassign mapping qualities
	  java -jar -Xmx"$SYS_MAX_MEM"g $gatk_dir/GenomeAnalysisTK.jar -T SplitNCigarReads -R $REF_HUMAN -I $second_star_out_dedupped_bam -o $split_bam -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 -U ALLOW_N_CIGAR_READS # --fix_misencoded_quality_scores

	  echo "[INFO] Variant calling" #HaplotypeCaller
	  java -jar -Xmx"$SYS_MAX_MEM"g $gatk_dir/GenomeAnalysisTK.jar -T HaplotypeCaller -R $REF_HUMAN -I $split_bam -dontUseSoftClippedBases -stand_call_conf 20.0 -stand_emit_conf 20.0 -o $vcf_out
    ...
    # $java_dir/java -jar $gatk_dir/GenomeAnalysisTK.jar -T VariantFiltration -R $REF_HUMAN -V $vcf_out -window 35 -cluster 3 -filterName FS -filter \""FS > 30.0\"" -filterName QD -filter \""QD < 2.0\"" -o $filtered_vcf_out
 	 	java -jar -Xmx"$SYS_MAX_MEM"g $gatk_dir/GenomeAnalysisTK.jar -T VariantFiltration -R $REF_HUMAN -V $vcf_out -window 35 -cluster 3 -o $filtered_vcf_out
