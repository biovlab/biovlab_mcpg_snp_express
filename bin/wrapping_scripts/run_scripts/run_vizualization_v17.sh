...
# TODO : DAVID & KEGG
	...
	# gerate kegg for deg, dmr-deg, mu-deg
	for (( k=0; k<${#type_kind[@]}; k++ )); do 
		for (( l=$k+1; l<${#type_kind[@]}; l++ )); do 
			work_file=${type_kind[$k]}"_vs_"${type_kind[$l]}

			DEG_with_FC=$final_result_dir_ge/$work_file".deg.for_KEGG_fig.txt"
			DEG_DMR_promoter=$integ_result_dir"/"$work_file".DEG_with_DMR.genelist"
			DEG_DMR_promoter_FC=$integ_result_dir"/"$work_file".DEG_with_DMR.genelist_FC"
			DEG_with_mutation=$integ_result_dir"/"$work_file".DEG_with_mutation.genelist"
			DEG_with_mutation_FC=$integ_result_dir"/"$work_file".DEG_with_mutation.genelist_FC"

			join <(sort $DEG_with_FC) <(sort $DEG_DMR_promoter) | awk '{OFS="\t";print $1,$2}' > $DEG_DMR_promoter_FC
			join <(sort $DEG_with_FC) <(sort $DEG_with_mutation)  | awk '{OFS="\t";print $1,$2}' > $DEG_with_mutation_FC

			$NEW_R_DIR/Rscript $bin_dir/create_kegg_fig_multiple_gene_lists.R $DEG_with_FC $DEG_DMR_promoter_FC $DEG_with_mutation_FC $work_file $viz_result_dir $viz_result_dir/$work_file".deg.for_KEGG_fig_maplist.txt" $viz_result_dir/$work_file".deg.for_KEGG_fig_map_namelist.txt"
	...
	cp $viz_result_dir"/"hsa* $viz_result_dir"/"ALL.DEG.list.DAVID_*.txt $viz_result_dir"/"ALL.DEG.KEGG.txt $viz_result_dir"/"*.deg.for_KEGG_fig_map_namelist.txt $viz_result_dir"/"*.deg.for_KEGG_fig_maplist.txt $final_result_dir_ge
	...
	for (( i=0; i<${#me_list[@]}; i++ )); do
		{
			...
			met_level_bed_file=$viz_result_dir/$temp_filename_only".met_level.bed";
...
if [ "$me_data_type" -eq "0" ] ; then # MBD
	ngs_all_config=$viz_result_dir/ngsplot_config_ME_all.txt
	echo -ne "" > $ngs_all_config
	for (( k=0; k<${#type_kind[@]}; k++ )); do 
		# variables
		ngs_subtype_config=$viz_result_dir/ngsplot_config_ME_${type_kind[$k]}.txt

		echo -ne "" > $ngs_subtype_config


		for (( i=0; i<${#me_list[@]}; i++ )); do 
			temp_filename_only=`basename \${me_list[$i]}`
			config_line="$mbd_result_dir/$temp_filename_only.sorted.bam\t-1\t$temp_filename_only"
		...
		for region in "cgi" "genebody" ; do #"enhancer" "dhs" "cgi" "exon"; do 
			ngs.plot.r -G hg19 -R $region -C $viz_result_dir/ngsplot_config_ME_${type_kind[$k]}.txt -O $viz_result_dir/ME_${type_kind[$k]}_$region 
			
			# convert pdf to png
			convert -density 150 $viz_result_dir/ME_${type_kind[$k]}_$region".avgprof.pdf" -quality 90 $viz_result_dir/ME_${type_kind[$k]}_$region".avgprof.png"
		done
	done

	cp $viz_result_dir/*.png $final_result_dir_me
	# for all sample
#	for region in "bed" ;do #"genebody" "enhancer" "dhs" "cgi"; do
#		/packages/test2/ngsplot-develop/bin/ngs.plot.r -G hg19 -R $region -C $ngs_all_config -O $viz_result_dir/ME_all_$region -E /data/project/mcpg/lib/region_info/intron.bed
#	done
...
#########################################
	for (( k=0; k<${#type_kind[@]}; k++ )); do 
		for (( l=$k+1; l<${#type_kind[@]}; l++ )); do 
			...
			cp $ge_result_dir/$work_file $viz_result_dir/$work_file".gene_only.annot.html" $final_result_dir_ge
		done
	done
...
all_type_file_list=()
for (( i=0; i<${#type_kind[@]}; i++ )); do 
	# This should be placed here to add all type file list
	subtype_ME_merged_file=$ME_all_merged"_"${type_kind[$i]}
	subtype_ME_merged_avg_file=$viz_result_dir/`basename \$subtype_ME_merged_file`"_avg"
	subtype_ME_merged_avg_10Mb_file=$subtype_ME_merged_avg_file"_10Mb"
	subtype_ME_merged_avg_1Mb_file=$subtype_ME_merged_avg_file"_1Mb"
 ...
 #  DEG_file_name=/usr/local/apache2/htdocs/biovlab_mcpg_snp_express/test_data/rna_paired_test_data/gene_exp/rna_seq/deg/Li_vs_Th.FC2.DEGs
	...
	#	DEG_for_circos=$circos_dir/$work_file".FC"$fold_change".DEGs.circos"
		...
		join -1 1 -2 4 <(sort $DEG_file_name) <(sort -k4,4 $REF_HUMAN_GENE_GENEBODY_INFO_SORTED) | awk '{print $2,$3,$4,$1}' OFS='\t' | sed -e 's/chr/hs/g' > $DEG_for_circos
		...
		bash $bin_dir/generate_circos_conf.sh 2 $file_list_10Mb $diff_file_10Mb $DEG_for_circos $CIRCOS_CONFIG_LOC $circos_min $circos_max $circos_diff_min $circos_diff_max $final_result_dir_me > $temp_circos_file
		...
		for hs in `cut -f1 $REF_HUMAN_GENOME_1M_BIN_BED | sort -k1,1 | uniq`; do
			...
			bash $bin_dir/generate_circos_conf.sh 2 $file_list_1Mb $diff_file_1Mb $DEG_for_circos $CIRCOS_CONFIG_LOC $circos_min $circos_max $circos_diff_min $circos_diff_max $final_result_dir_me > $temp_circos_file
			circos -conf $temp_circos_file -outputdir $circos_dir -outputfile $work_file"_"$hs".circos"
		done
	done
done
...
join -1 1 -2 4 <(sort $GDEG"_genelist") <(sort -k4,4 $REF_HUMAN_GENE_GENEBODY_INFO_SORTED) | awk '{print $2,$3,$4,$1}' OFS='\t' | sed -e 's/chr/hs/g' > $GDEG_for_circos
...
bash $bin_dir/generate_circos_conf.sh ${#type_kind[@]} `my_join ";" ${all_type_file_list[@]}` $diff_all_file $GDEG_for_circos $CIRCOS_CONFIG_LOC $circos_min $circos_max $circos_diff_min $circos_diff_max $final_result_dir_me > $temp_circos_file
...
for hs in `cut -f1 $REF_HUMAN_GENOME_1M_BIN_BED | sort -k1,1 | uniq`; do
	...
	bash $bin_dir/generate_circos_conf.sh ${#type_kind[@]} `my_join ";" ${all_type_file_list_1Mb_hs[@]}` $diff_all_file_hs $GDEG_for_circos $CIRCOS_CONFIG_LOC $circos_min $circos_max $circos_diff_min $circos_diff_max $final_result_dir_me > $temp_circos_file
...
cp $circos_dir/*.png $circos_dir/*.html $final_result_dir_me
...
#################################################
 for(( k=0; k<${#type_kind[@]}; k++)); do
	for(( l=$k+1; l<${#type_kind[@]}; l++)); do
		...
		# variables
		first_class_file=(); second_class_file=()
		first_class_file_header=(); second_class_file_header=()
...
#		class2_header_string=$(IFS=$'\t'; echo "${second_class_file_header[*]}")
		...
		# draw heatmap
		$R_DIR/Rscript $bin_dir/make_heatmap3_ver2_error_handling.r $subtype_dmr_with_value".promoter.heatmap.input" `my_join "," ${first_class_file_header[@]}`","`my_join "," ${second_class_file_header[@]}` "Class" ${type_kind[$k]}","${type_kind[$l]} $viz_result_dir"/"$work_file".DMR_pro.heatmap.png" "DMR_overlap_with_Promoter"
...
# GDSNP per sample : boxplot
	...
	cp $viz_result_dir/ALL_SNP_boxplot_genomic.png $viz_result_dir/ALL_SNP_boxplot_cpgi.png $final_result_dir_mu
...
TEMP_NUM_CPUS=6 # NOTE: below takes 10GB per CPU.
...
COMMENT
				...
				let count+=1; [[ $((count%$TEMP_NUM_CPUS)) -eq 0 ]] && wait
			done; wait
...
cp $viz_result_dir/"mu_rate.png" $final_result_dir_mu
...
COMMENT
		top30_degs_genelist=$integ_result_dir/$work_file".DEG_w_mut_onco_top30.txt"
...
COMMENT
	top30_degs_genelist=$integ_result_dir/ALL.DEG_w_mut_onco_top30.txt
...
cp $viz_result_dir/*.onco.png $final_result_dir_mu
...
echo "[INFO] Generate final json"
# generate data string for final json
function add_to_data_string() {
	data_string=$data_string"^"$data_type";"$work_file";"`my_join ";" "$@"`;
}

# data string should be formatted as below
...
for((k=0; k<${#type_kind[@]}; k++)); do
	for((l=$k+1; l<${#type_kind[@]}; l++)); do
		...
		# add to data string
		if [ $l -eq 1 ] && [ $k -eq 0 ] && [ "$data_type" == "gene_expression" ]; then
			# title
			data_string="$data_type;$work_file;normal;All sample status;12;title"
		else
			add_to_data_string normal "All sample status" 12 title
		fi

		# MA plot
		add_to_data_string normal $work_file".MA_plot.png" 6 image

		# box plot
		add_to_data_string normal $work_file".norm.boxplot.png" 6 image 

		# NGS plot for RNA-seq
		if [[ $ge_data_type -eq 0 ]]; then
			## title	
			add_to_data_string normal "Region specific average gene expression" 12 title

			add_to_data_string normal "GE_"${type_kind[$k]}"_genebody.avgprof.png" 6 image
			add_to_data_string normal "GE_"${type_kind[$l]}"_genebody.avgprof.png" 6 image
			add_to_data_string normal "GE_"${type_kind[$k]}"_cgi.avgprof.png" 6 image
			add_to_data_string normal "GE_"${type_kind[$l]}"_cgi.avgprof.png" 6 image
		fi

		## title	
		add_to_data_string tag "Differentially expressed genes (DEG)s" 12 title DEG/DEG

		# DEG Top 100 table
		add_to_data_string normal $work_file".DEG.Top100.table.txt" 12 table

		#dummy
		add_to_data_string dummy "dummy" 6 dummy

		# Total table
		add_to_data_string normal $work_file".Total.table.txt" 3 download 
		# DEG table
		add_to_data_string normal $work_file".DEG.table.txt" 3 download 

		## title	
		add_to_data_string normal "Plots of DEGs" 12 title

		# volcano plot
		add_to_data_string normal $work_file".volcano.png" 4 image

		# DEG Top 100 heatmap
		add_to_data_string normal $work_file".DEG.MGD.heatmap.png" 4 image

		# DEG PCA
		add_to_data_string normal $work_file".DEG.PCA.png" 4 image

		## title	
		add_to_data_string tag "Gene set analyses (GSA)s" 12 title GENE_SET/DEG

		#dummy
		add_to_data_string dummy "dummy" 2 dummy 

		# DAVID
		temp_link=$(cat $final_result_dir_ge"/"$work_file".DEG.list.DAVID_summary.txt")
		add_to_data_string link "Functional analysis with DEGs" 8 link $temp_link 

		#dummy
		add_to_data_string dummy "dummy" 2 dummy 

		## title	
		add_to_data_string tag "Enriched Pathways based on (1:DEGs, 2:DMR-DEGs, 3:Mutated DEGs)" 12 title PATHWAY

		# KEGG
		# read map list to array
		IFS=$'\r\n' map_list=($(cat $final_result_dir_ge/$work_file".deg.for_KEGG_fig_maplist.txt"))
		IFS=$'\r\n' map_namelist=($(cat $final_result_dir_ge/$work_file".deg.for_KEGG_fig_map_namelist.txt"))

		for (( i=0; i<${#map_list[@]}; i++ )); do
			# map subtitle
			add_to_data_string normal ${map_namelist[$i]} 12 title

			add_to_data_string normal ${map_list[$i]}"."$work_file".png" 4 image
			add_to_data_string normal ${map_list[$i]}"."$work_file".deg_dmr.png" 4 image
			add_to_data_string normal ${map_list[$i]}"."$work_file".deg_mu.png" 4 image
		done

		## title	
		add_to_data_string normal "Visualization all analyzed data on UCSC genome browser" 12 title

		#dummy
		add_to_data_string dummy "dummy" 2 dummy 

		# UCSC link
		temp_link=$(cat $final_result_dir_ge"/"$work_file".UCSC_hub_link.txt")
		add_to_data_string link "Browse analyzed data on UCSC genome browser" 8 link $temp_link 
		...
		if [ "$me_data_type" -eq "0" ]; then
			## title	
			add_to_data_string normal "Region specific average methylation" 12 title

			add_to_data_string normal "ME_"${type_kind[$k]}"_genebody.avgprof.png" 6 image
			add_to_data_string normal "ME_"${type_kind[$l]}"_genebody.avgprof.png" 6 image
			add_to_data_string normal "ME_"${type_kind[$k]}"_cgi.avgprof.png" 6 image
			add_to_data_string normal "ME_"${type_kind[$l]}"_cgi.avgprof.png" 6 image
		elif [ "$me_data_type" -eq "3" ]; then
			## title	
			add_to_data_string normal "Pair-wise correlation" 12 title

			add_to_data_string normal $work_file".corr.png" 12 image
		fi

		## title	
		add_to_data_string normal "Pair-wise methylation density & Hierachcal clusturing with heatmap" 12 title

		# Density plot
		add_to_data_string normal $work_file".DMR.density.png" 6 image

		# promoter heatmap
		add_to_data_string normal $work_file".DMR_pro.heatmap.png" 6 image
	
		## title	
		add_to_data_string normal "Regional DMR bin stats & PCA" 12 title

		# DMR barplot
		add_to_data_string normal $work_file".DMR.barplot.png" 6 image

		# DMR PCA
		add_to_data_string normal $work_file".DMR.PCA.png" 6 image

		## title	
		add_to_data_string tag "DEGs having DMRs in thier promoters" 12 title DEG/DMR-DEG

		# DEG Top 100 table
		add_to_data_string normal $work_file".DEG_with_DMR.table.txt.top100.txt" 12 table

		#dummy
		add_to_data_string dummy "dummy" 9 dummy

		# DEG table
		add_to_data_string normal $work_file".DEG_with_DMR.table.txt" 3 download 


		## title	
		add_to_data_string normal "Genome wide methylation landscape with DEGs" 12 title

		# circos
		add_to_data_string normal $work_file".circos.png" 12 image 

		## title	
		add_to_data_string tag "Gene set analyses (GSA)s" 12 title GENE_SET/DMR-DEG

		#dummy
		add_to_data_string dummy "dummy" 2 dummy 

		# DAVID
		temp_link=$(cat $final_result_dir_me"/"$work_file".DEG_with_DMR.genelist.DAVID_summary.txt")
		add_to_data_string link "Functional analysis with DEGs" 8 link $temp_link 
		...
		## title	
		add_to_data_string normal "SNP and Indel status" 12 title

		# total count
		add_to_data_string normal $work_file".snp.tot.count.png" 6 image 
		add_to_data_string normal $work_file".indel.tot.count.png" 6 image
		
		# overlap AF
		add_to_data_string normal $work_file".snp.overlap.AF.png" 6 image
		add_to_data_string normal $work_file".indel.overlap.AF.png" 6 image

		## title	
		add_to_data_string normal "TS/TV QUAL" 12 title 

		# ts_tv qual
		add_to_data_string normal $work_file".ts_tv.qual.classA.png" 4 image
		add_to_data_string normal $work_file".ts_tv.qual.classB.png" 4 image
		add_to_data_string normal $work_file".ts_tv.qual.classAB.png" 4 image

		## title	
		add_to_data_string normal "Indel distribution" 12 title 

		# indel dist
		add_to_data_string normal $work_file".indel_dist.classA.png" 4 image
		add_to_data_string normal $work_file".indel_dist.classB.png" 4 image
		add_to_data_string normal $work_file".indel_dist.classAB.png" 4 image

		## title	
		add_to_data_string normal "Depth disribution" 12 title 

		# depth dist
		add_to_data_string normal $work_file".depth_dist.classA.png" 4 image 
		add_to_data_string normal $work_file".depth_dist.classB.png" 4 image 
		add_to_data_string normal $work_file".depth_dist.classAB.png" 4 image

		## title	
		add_to_data_string normal "Substitution types" 12 title 

		# subst type
		add_to_data_string normal $work_file".subst_type.classA.png" 4 image
		add_to_data_string normal $work_file".subst_type.classB.png" 4 image
		add_to_data_string normal $work_file".subst_type.classAB.png" 4 image

		## title	
		add_to_data_string tag "DEGs having mutations" 12 title DEG/Mutated-DEG

		# DEG Top 100 table
		add_to_data_string normal $work_file".DEG_with_mutation.table.txt.top100.txt" 12 table

		#dummy
		add_to_data_string dummy "dummy" 9 dummy

		# DEG table
		add_to_data_string normal $work_file".DEG_with_mutation.table.txt" 3 download 

		## title	
		add_to_data_string normal "Mutation occurrence on DEGs" 12 title

		# oncoprint
		add_to_data_string normal $work_file".onco.png" 12 image 

		## title	
		add_to_data_string tag "Gene set analyses (GSA)s" 12 title GENE_SET/Mutated-DEG

		#dummy
		add_to_data_string dummy "dummy" 2 dummy 

		# DAVID
		temp_link=$(cat $final_result_dir_mu"/"$work_file".DEG_with_mutation.genelist.DAVID_summary.txt")
		add_to_data_string link "Functional analysis with DEGs" 8 link $temp_link 
...
if [ ${#type_kind[@]} -gt 2 ]; then
	...
	## title	
	add_to_data_string tag "Group DEGs" 12 title DEG/DEG
	...
	## title	
	add_to_data_string normal "Plots of Group DEGs" 12 title

	# GDEG Top 100 heatmap
	add_to_data_string normal $work_file".DEG.MGD.heatmap.png" 6 image

	# GDEG PCA
	add_to_data_string normal $work_file".DEG.PCA.png" 6 image
	...
	for (( i=0; i<${#map_list[@]}; i++ )); do
		...
		#add_to_data_string normal ${map_list[$i]}"."$work_file".png" 2 image
		add_to_data_string normal ${map_list[$i]}"."$work_file".png" 4 image
		add_to_data_string normal ${map_list[$i]}"."$work_file".deg_dmr.png" 4 image
		add_to_data_string normal ${map_list[$i]}"."$work_file".deg_mu.png" 4 image
	done
	...
	## title	
	add_to_data_string normal "All clase-wise Hierachcal clusturing with headmap" 12 title

	# GDMR promoter heatamp
	add_to_data_string normal $work_file".DMR_pro.heatmap.png" 12 image 
	...
	## title	
	add_to_data_string tag "Group DEGs having DMRs in promoter" 12 title DEG/DMR-DEG
	...
	## title	
	add_to_data_string tag "Network analysis" 12 title NETWORK

	#dummy
	add_to_data_string dummy "dummy" 2 dummy 

	# cyto 
	add_to_data_string normal cyto.txt 8 link
	...
	data_type="mutation"
	## title	
	add_to_data_string tag "Group DEGs having mutations" 12 title DEG/Mutated-DEG
	...
	# mu_rate
	add_to_data_string normal mu_rate.png 12 image 

fi

# generate final json
python $bin_dir/formatting_final_json_v3.py $final_result_root_dir/"final_result.json" $data_string $viz_result_dir/json_debug.txt

#formating the jason for cyto panel
python $bin_dir/formatting_data.py $integ_result_dir/Cyto_panel_TF_TG.txt $integ_result_dir/Cyto_panel_TG_exp.txt $integ_result_dir/Cyto_panel_TG_Info.txt $integ_result_dir/Cyto_panel_TG_promoter_methyl.txt $final_result_dir_me/cyto.txt $final_result_dir_me/cyto_extended.txt
