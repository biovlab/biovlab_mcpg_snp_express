...
################################
	...
	for((k=0; k<${#type_kind[@]}; k++)); do
		for((l=$k+1; l<${#type_kind[@]}; l++)); do
			...
			# Total table
			json_elem $work_file".Total.table.txt" 3 download 1 ;

			# DEG table
			json_elem $work_file".DEG.table.txt" 3 download 1 ;
			...
			if [ ${#map_list[@]} -gt 0 ]; then
				...
				json_link_elem "Functional analysis with DEGs" 12 $temp_link link 1 ;
	...
	if [ ${#type_kind[@]} -gt 2 ]; then
      ...
      # KEGG
				...
				temp_link=$(cat $final_result_dir_ge"/"$work_file".DEG.list.DAVID_summary.txt")
				json_link_elem "Functional analysis with DEGs" 12 $temp_link link 0 ;
      	#json_elem $work_file".DEG.list.DAVID_summary.txt" 12 link 0 ;
			fi
	...
	if [ "${#type_kind[@]}" -gt "2" ]; then
		for((k=0;k<${#type_kind[@]};k++)); do
			for((l=$k+1;l<${#type_kind[@]};l++)); do
				...
				# UCSC link
				temp_link=$(cat $final_result_dir_inte"/"$work_file".UCSC_hub_link.txt")
				json_link_elem "Browse analyzed data on UCSC genome browser" 12 $temp_link link 1 ;
