...
#	ge_basename_list_string=$(IFS=$'\t'; echo "${ge_basename_list[*]}")
	ge_basename_list_string=`my_join "/" ${ge_basename_list[@]}`
 	...
 	{
		echo -e "GeneSymbol\t"$ge_basename_list_string"\tP.val\tadj.P.val" | tr '/' '\t' ;

		awk 'NR>1{if($(NF) != "nan"){split($1, tokens, "_"); print tokens[1]"\t"tokens[2]"\t"tokens[3]"\t",$0}}' $integ_result_dir/GE_MAT.kw.txt | bedtools intersect -wb -wa -a $temp_genesymbol_pos_file -b - | cut --complement -f1-3,5-9 | sort -k1,1 | uniq | sort -k$((3+${#type_list[@]})),$((3+${#type_list[@]}))g ;
...
# methyl average for cytoscape
  ...
  awk 'BEGIN{FS=OFS="\t"} FILENAME==ARGV[1]{arr[$1]=$0;next}
	...
	cp $ALL_avg_for_cyto_data $final_result_dir/cyto.txt
...
<<'COMMENT'
	# cyto promoter methyl
	template_TG_methyl=$integ_result_dir"/Cyto_Template_TG_methyl.txt"
	cut -f2,$((${#type_kind[@]}+3))- $template_TF_TG | sort -k1,1 | uniq > $template_TG_methyl
COMMENT
...
<<'COMMENT'
	cut -f2,$((${#type_kind[@]}+3))- $template_TF_TG | sort -k1,1 | uniq >> $panel_promoter		
COMMENT
...
<<'COMMENT'
	awk 'BEGIN{FS=OFS="\t"}FILENAME==ARGV[1]{arr[$1]=$2}FILENAME==ARGV[2]{if($1 in arr){printf "%s\t%s", $1"_Promoter", arr[$1]; for(i=2;i<=NF;i++){printf "\t%s", $(i)};printf "\n"}else{printf "%s\t%s", $1"_Promoter", "NA"; for(i=2;i<=NF;i++){printf "\t%s", $(i)}}}' $DEG_Promoter_methyl_CORR $template_TG_methyl >> $panel_promoter
COMMENT
	# TG info file
...
<<'COMMENT'
	DEGs_adj_pval_MGD=$integ_result_dir/DEG_adj_pval_MGD.txt
	DEGs_adj_pval_MGD_temp=$integ_result_dir/DEG_adj_pval_MGD.txt.tmp

	if [ "$ge_data_type" -eq "1" ]; then
		awk 'BEGIN{FS=OFS="\t"}FILENAME==ARGV[1]{arr[$1]=$2}FILENAME==ARGV[2]{if($2 in arr){print $2, $1, arr[$2]}}' $DEG_Promoter_methyl_CORR $ge_result_dir"/"${type_kind[0]}"_vs_"${type_kind[1]}".Total.table.txt" > $DEGs_adj_pval_MGD_temp
	else
		cp $DEG_Promoter_methyl_CORR $DEGs_adj_pval_MGD_temp
	fi
    ...
    for((l=$k+1;l<${#type_kind[@]};l++)); do
      work_file=${type_kind[$k]}"_vs_"${type_kind[$l]}
			header_list+=($work_file)
      if [ "$ge_data_type" -eq "1" ]; then # cel
				# Gene_symbol	ID	CORR
				awk 'BEGIN{FS=OFS="\t"}FILENAME==ARGV[1]{arr[$2]=$0;next}FILENAME==ARGV[2]{if($1 in arr){arr2[$1]=(-1*log($(NF))/log(10))}}END{for(x in arr){if(x in arr2){print arr[x], arr2[x]}else{print arr[x], "NA"}}}' $DEGs_adj_pval_MGD_temp $ge_result_dir"/"$work_file".Total.table.txt" > $DEGs_adj_pval_MGD	
			else
				# Gene_symbol	CORR
				awk 'BEGIN{FS=OFS="\t"}FILENAME=ARGV[1]{arr[$1]=$0;next}FILENAME=ARGV[2]{if($1 in arr){arr2[$1]=(-1*log($(NF))/log(10))}}END{for(x in arr){if(x in arr2){print arr[x], arr2[x]}else{print arr[x],"NA"}}}' $DEGs_adj_pval_MGD_temp $ge_result_dir"/"$work_file".Total.table.txt" > $DEGs_adj_pval_MGD
			fi

			mv $DEGs_adj_pval_MGD $DEGs_adj_pval_MGD_temp
		done
	done
	...
	#for all-class
	if [ "$ge_data_type" -eq "1" ]; then
		header_string="Gene;ID;Corr;"`my_join ";" ${header_list[@]}`";ALL_Class"

		awk 'BEGIN{FS=OFS="\t"}FILENAME==ARGV[1]{arr[$1]=(-1*log($(NF))/log(10));next}FILENAME==ARGV[2]{if($1 in arr){print $0, arr[$1]}else{print $0,"NA"}}' $integ_result_dir/ALL.Total.table.txt $DEGs_adj_pval_MGD_temp > $DEGs_adj_pval_MGD		
		
		echo $header_string | tr ";" "\t" > $panel_TG_Info
		sort -k1,1 $DEGs_adj_pval_MGD >> $panel_TG_Info

	else
		header_string="Gene;Corr;"`my_join ";" ${header_list[@]}`";ALL_Class"
	
		awk 'BEGIN{FS=OFS="\t"}FILENAME==ARGV[1]{arr[$1]=$0;next}FILENAME==ARGV[2]{if($1 in arr){arr2[$1]=(-1*log($(NF))/log(10))}}END{for(x in arr){if(x in arr2){print arr[x], arr2[x]}else{print arr[x],"NA"}}}' $DEGs_adj_pval_MGD_temp $integ_result_dir/ALL.Total.table.txt > $DEGs_adj_pval_MGD
