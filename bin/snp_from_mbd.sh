...
filename_only=$3
	...
	vcfutils.pl splitchr -l 10000000000 $REF_HUMAN".fai" | xargs --verbose -I {} -P $SYS_NUM_CPUS bash -c 'samtools mpileup -uf "$2" -r "$1" "$3" | bcftools view -bcvg - > "$4"/"$5".part-"$1".bcf' -- {} $REF_HUMAN $sorted_bam $result_dir $filename_only
