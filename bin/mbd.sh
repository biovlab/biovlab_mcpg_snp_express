...
if [ $single_pair == "single" ]; then
	cd $result_dir
	STAR --genomeDir $REF_HUMAN_DIR --readFilesIn $input_fastq1 --runThreadN $NUM_CPUS --alignIntronMax 1 --outFileNamePrefix $result_dir/$input_fq1_filename_only"." --alignEndsType EndToEnd --seedSearchStartLmax 20 --outFilterScoreMinOverLread 0 --outFilterMatchNminOverLread 0 # --outFilterMatchNmin 15 # --alignEndsType EndToEnd --seedSearchStartLmax 30 --outFilterScoreMinOverLread 0 --outFilterMatchNminOverLread 0 options are for increasing sensitivity for less than 50bp reads. MBDseq ICBP 36bp length 
	cd $WORK_DIR
else
	cd $result_dir
	STAR --genomeDir $REF_HUMAN_DIR --readFilesIn $input_fastq1 $input_fastq2 --runThreadN $NUM_CPUS --alignIntronMax 1 --outFileNamePrefix $result_dir/$input_fq1_filename_only"." --alignEndsType EndToEnd --seedSearchStartLmax 20 --outFilterScoreMinOverLread 0 --outFilterMatchNminOverLread 0 # --outFilterMatchNmin 15 # --alignEndsType EndToEnd --seedSearchStartLmax 30 --outFilterScoreMinOverLread 0 --outFilterMatchNminOverLread 0 options are for increasing sensitivity for less than 50bp reads. MBDseq ICBP 36bp length 
	cd $WORK_DIR
fi
