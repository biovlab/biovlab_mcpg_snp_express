...
# parameter check
if len(sys.argv) < 6 :
	print "[USAGE : python bin/merge_methyl.py [methyl sample_file_list] [gene expression sample file list] [correlation threshold] [merge out_file] [stat out file]"
        sys.exit(0)
...
while 1:
	...
	#merge_out_file_fd.write(line[:-1])
	...
	#for file_num in range(len(methyl_file_list)):
		...
		if (len(tokens) < 11): 
			print "[Error] Input methyl sample has less then 11 column! Please check the input file : " + methyl_file_list[file_num]
			print "[Error] The input line is : " + line[:-1]
			sys.exit(0)
		...
		#fixed_key 	= chr_num+"~"+ bin_start +"~"+ range_start + "~" + range_end + "~" + refseq + "~" + gene_symbol + "~" + range_kind

		# print as original format
		if (file_num == 0):
			chr_num 	= tokens[0]
			bin_start 	= str(tokens[1])
			bin_end	= str(tokens[2])
			methyl_count	= tokens[3]
			...
			pre_key = chr_num + "\t" + bin_start + "\t" + bin_end 
			pos_key = "\t" + range_start + "\t" + range_end + "\t" + strand + "\t" + refseq + "\t" + gene_symbol + "\t" + range_kind 
...
for key in data.keys():
	...
	if (data[key]['count'] > threshold):
		...
		num_methyls_within_tss_range = num_methyls_within_tss_range + 1
