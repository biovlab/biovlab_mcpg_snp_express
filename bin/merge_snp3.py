...
for range_kind in num_snps_within_ranges.keys():
	...
	stat_out_file_fd.write("(4). % of SNP over rank correlation threshold +-" + str(corr_threshold) + " (3)/(1) : " + str(float(num_snp_over_threshold[range_kind])/float(num_snps_having_corr[range_kind]) * 100) + "%\n")
	try:
