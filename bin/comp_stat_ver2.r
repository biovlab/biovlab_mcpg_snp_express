...
getCorrelation(meth, plot=TRUE)
	...
	cat("[Error] Fail to get correlation\n")
	print(err)
}, finally={
})
dev.off()
...
tryCatch({
	...
	cat("[Error] Fail to calculate DMR")
  ...
  dmr_merge_out_file = paste(result_dir,"/",comp_sample,".DMR_merge_list.txt",sep="")
	...
	write.table(empty_data, dmr_out_file, quote=FALSE, row.names=FALSE, col.name=FALSE, sep="\t")

  write.table(empty_data, hyper_dmr_out_file, quote=FALSE, row.names=FALSE, col.name=FALSE, sep="\t")

  write.table(empty_data, hypo_dmr_out_file, quote=FALSE, row.names=FALSE, col.name=FALSE, sep="\t")

	write.table(empty_data, dmr_merge_out_file, quote=FALSE, row.names=FALSE, col.name=FALSE, sep="\t")
}, finally={
})
