...
# parameter check
if len(sys.argv) < 7 :
	print "[USAGE : python bin/profile_AF_GE.py [merged ME_GE_COR file] [out file] [sample num] [correlation threshold] [out matrix file for theshold] [out matrix file for PC NC ratio]"
        sys.exit(0)
...
range_kind_index=sample_num+sample_num+8-1
corr_index=(sample_num*3)+10-1
...
for range_kind in range_count.keys():
	...
	try:
		...
		stat_out_file_fd.write("(1). Number of bins within " + range_kind + " range : 0\n")
	try:
		stat_out_file_fd.write("(2). Number of bins having Spermans's rho with gene expression among (1) :" + str(range_having_corr_count[range_kind]) + "\n")
	except KeyError:
		stat_out_file_fd.write("(2). Number of bins having Spermans's rho with gene expression among (1) : 0\n")
	try:
		stat_out_file_fd.write("(3). Number of bins having Spermans's rho(rank correlation) over +- " + str(corr_threshold) + " among (2) :" + str(range_corr_over_threshold_count[range_kind]) + "\n")
	except KeyError:
		stat_out_file_fd.write("(3). Number of bins having Spermans's rho(rank correlation) over +- " + str(corr_threshold) + " among (2) : 0\n")
	try:
		stat_out_file_fd.write("(4). % of bins over rank correlation threshold +-" + str(corr_threshold) + " (3)/(2) : " + str(float(range_corr_over_threshold_count[range_kind])/float(range_having_corr_count[range_kind]) * 100) + "%\n")
	except KeyError:
		stat_out_file_fd.write("(3). Number of bins having Spermans's rho(rank correlation) over +- " + str(corr_threshold) + " among (2) : 0\n")
	try:
