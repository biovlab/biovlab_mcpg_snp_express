...
# parameter check
if len(sys.argv) < 4 :
	print "[USAGE : python bin/get_GDSNP.py [merged snp file] [class num] [class list] [GDSNP bed file]"
        sys.exit(0)

# variables
merged_snp_file_fd		= open(sys.argv[1], "r")
gdsnp_bed_file_fd		= open(sys.argv[4], "w")
...
for line in merged_snp_file_fd.readlines():
	...
	for af1_index in range(len(af1_list)):
		...
		if (af1_list[af1_index] !='-'):
			if(float(af1_list[af1_index]) != 0.0):
	...
	for sample_class_first in sample_class_set:
		...
		if (line_class_exist_count[sample_class_first] >= class_total_count_mul_GDSNP_exist_pct_in_the_subtype[sample_class_first]):
			...
			if (is_GDSNP == 1):
				print line[:-1]
				gdsnp_bed_file_fd.write(chr_num + "\t" + start + "\t" + end + "\t" + sample_class_first + "\n")
				break

gdsnp_bed_file_fd.close()
